...
var CustomFunctions = /** @class */ (function () {
    ...
    CustomFunctions.jsonToCsv = function (json, headers) {
        if (Object.keys(json).length === 0)
            ...
            return [[]];
        var headersToUse = headers.map(function (header) { return "root.".concat(header); });
        var csv = [];
        var map = {};
        ...
        ;
        function setDepthInMap(currentData, depth, parent) {
            if (currentData === void 0) { currentData = json; }
            if (depth === void 0) { depth = 1; }
            var _loop_8 = function (data) {
                var dataWhichIsNotArray = {};
                headersToUse.filter(function (header) { return header.split('.').length === depth + 1; }).forEach(function (header) {
                    var _a;
                    var thisParameter = header.split('.')[depth];
                ...
                var nextDepthLevelHeaders = headersToUse.filter(function (header) { return header.split('.').length > depth + 1; });
                var paths = [];
                nextDepthLevelHeaders.forEach(function (header) {
                    var currentLevelToLookUp = header.split('.')[depth];
                    if (!paths.includes(currentLevelToLookUp))
                        paths.push(currentLevelToLookUp);
                });
                if (paths.some(function (pathHeader) { return Array.isArray(data[pathHeader]); })) {
                    var _loop_9 = function (pathHeader) {
                        if (data[pathHeader].length === 0)
                            data[pathHeader].push({});
                        for (var _b = 0, _c = data[pathHeader]; _b < _c.length; _b++) {
                            var child = _c[_b];
                            fillUndefinedValues(headersToUse.filter(function (header) { return header.split('.').length === depth + 2; }).map(function (header) { return header.split('.').pop(); }), child);
                        }
                        ;
                        if (!Object.keys(data[pathHeader]).every(function (key) { return data[pathHeader][key] === undefined; })) {
                            setDepthInMap(data[pathHeader], depth + 1, dataWhichIsNotArray);
                        }
                        ;
                    };
                    for (var _a = 0, paths_1 = paths; _a < paths_1.length; _a++) {
                        var pathHeader = paths_1[_a];
                        _loop_9(pathHeader);
                    }
                    ;
                }
        ...
        setDepthInMap();
        var allData = [];
        for (var i = 0; i < map[maxDepth.toString()].length; i++) {
            allData.push([]);
            var row = map[maxDepth.toString()][i];
            var target = row;
            do {
        ...
        allData.forEach(function (row) {
            csv.push(row.sort(function (_a, _b) {
                var value_1 = _a[0], attribute_1 = _a[1];
                var value_2 = _b[0], attribute_2 = _b[1];
