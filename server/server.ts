...
app.get("/myanimelist/:type/:username/", async (req: express.Request, res: express.Response) => {
	...
	try {
		const response: Response = await fetch(`${MAL_API_URL}users/${username}/${type}?limit=${limit ?? 100}&sort=list_updated_at&offset=${offset ?? 0}&fields=list_status,genres,num_episodes,num_chapters,nsfw,rank&nsfw=${nsfw ?? false}${status ? "&status=" + status : ""}`, {
			headers: {
...
app.post("/contents/:type/", async (req: express.Request, res: express.Response) => {
	const type = req.params.type as string;
...
app.delete("/contents/:type/", async (req: express.Request, res: express.Response) => {
	const type = req.params.type as string;
...
app.put("/contents/:type/", async (req: express.Request, res: express.Response) => {
	const type = req.params.type as string;
